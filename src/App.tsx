      ...
      <div style={{ padding: 16, maxWidth: 800, margin: "0 auto" }}>
        {loading ? (
          ...
          <>
            ...
            {/* GOLEADORES */}
            {busqueda.length < 2 && tab === "goleadores" && (
              <div>
                ...
                {goleadores.map((g, i) => (
                  <div key={i} style={{ background: "#0d1320", border: "1px solid #1e2d45", borderRadius: 8, padding: "12px 16px", marginBottom: 8, display: "grid", gridTemplateColumns: "32px auto 1fr auto", alignItems: "center", gap: 12 }}>
                    ...
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontFamily: "Barlow Condensed", fontSize: 24, fontWeight: 800, color: "#ff4d6d" }}>{g.goles} ⚽</div>
                      {g.asistencias > 0 && <div style={{ fontSize: 11, color: "#00e5ff" }}>{g.asistencias} asist.</div>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* BUSQUEDA */}
            {busqueda.length > 1 && filtrar(partidos).map(p => renderPartido(p))}
          </>
        )}
      </div>
    </div>
